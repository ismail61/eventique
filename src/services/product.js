...
const getAllItems = async (query, offset, limit) => {
    try {
...
const getItemsCount = async (query) => {
    try {
        return await ItemModel.countDocuments(query);
    } catch (error) {
        console.log(error);
        return null;
    }
...
module.exports = { getAllItems, addItem, findItem, getItemsCount, updateItem }